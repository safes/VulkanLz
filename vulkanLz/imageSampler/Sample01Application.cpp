...
void Sample01Application::createTextureImage()
{
	...
	stbi_uc *pixels = stbi_load("statue.jpg",&texWidth,&texHeight,&texChannels,
		STBI_rgb_alpha);
...
void Sample01Application::createImage(uint32_t width, uint32_t height, VkFormat format,
...
void Sample01Application::createImageViews()
{
	...
	for (auto& image : swapChainImages) {
		swapChainImageView[i++] = createImageView(image, swapchainFormat);
	}
...
void Sample01Application::createGraphicPipeline()
{
	...
	std::vector< VkVertexInputAttributeDescription> attrDesc = {
		{0,0,VK_FORMAT_R32G32_SFLOAT,offsetof(vertexIn,pos)},
...
void Sample01Application::createRenderPass()
{
	...
	renderPassInfo.pDependencies = nullptr;// &subpassDepend;
...
void Sample01Application::updateUniformBuffer(uint32_t currentImage)
{
	...
	uniformObject ubo{};
	ubo.Model = glm::rotate(glm::mat4(1.0),time*glm::radians(90.f),
		glm::vec3(0.,0.,1.));
	...
	VkImageLayout oldLayout, VkImageLayout newLayout)
{
	...
	barrior.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
	...
	vkCmdPipelineBarrier(commandbuffer, 0, 0, 0, 0, nullptr, 0, nullptr, 1, &barrior);
...
void Sample01Application::createCommandBuffers()
{
	...
	for (auto& commandbufffer : commandBuffers) {
		...
		vkCmdBindIndexBuffer(commandbufffer, indexBuffer, 0, VK_INDEX_TYPE_UINT16);
